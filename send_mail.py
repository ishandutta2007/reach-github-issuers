...
def get_msgHtml(username, fullname, issue_url):
    f = codecs.open("help_request.html", 'r')
    msgHtml = f.read()
    if fullname is not None:
        msgHtml = msgHtml.replace('[fullname]', fullname.split(' ')[0].title())
    else:
        msgHtml = msgHtml.replace('[fullname]', username.title())
    msgHtml = msgHtml.replace('[issue]', issue_url)
    msgHtml = msgHtml.replace('[sender]', "Ishan")
    return msgHtml

def sendmail(attribute):
    username = attribute[0]
    issue_url = attribute[1]
    issue_title = attribute[2]
    fullname = attribute[3]
    email = attribute[4].strip()
    if email=='':
        print('No email found for', username)
        return
    print(('Sending email to %s' % (str(email))))

    to = email
    sender = "you.email"
    subject = "Re:"+issue_title.title()
    msgPlain = ""
    msgHtml = get_msgHtml(username, fullname, issue_url)
